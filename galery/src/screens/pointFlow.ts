...
export const renderNextPoint = (): HTMLElement => {
  ...
  card.innerHTML = `
    ...
    <div class="point-layout__actions point-layout__actions--inline">
      <button class="button icon-button" data-action="map" aria-label="Открыть карту этажа">
        ...
        <span class="icon-button__text">Карта этажа</span>
      </button>
      <button class="button primary" data-action="scan">Отсканировать QR код</button>
    </div>
    <p class="point-layout__hint">Отсканируйте QR-код точки, чтобы открыть следующую часть маршрута</p>
    <button class="button secondary point-layout__route" data-action="route">Открыть весь маршрут</button>
  `
